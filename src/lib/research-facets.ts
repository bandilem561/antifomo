...
const INDUSTRY_RULES: FacetRule[] = [
...
export type ResearchPerspective = "all" | "regional" | "client_followup" | "bidding" | "ecosystem";

function normalizeText(value: string | null | undefined): string {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function collectResearchText(entry: ApiKnowledgeEntry): string {
...
function getReport(entry: ApiKnowledgeEntry): {
  client_peer_moves?: string[];
  winner_peer_moves?: string[];
  competition_analysis?: string[];
  five_year_outlook?: string[];
...
function matchRule(text: string, rules: FacetRule[], fallback: string): string {
...
export function getResearchActionTypeLabel(entry: ApiKnowledgeEntry, fallback = "研报"): string {
  const payload = (entry.metadata_payload || {}) as { card?: { action_type?: string } };
  const actionType = payload.card?.action_type || "";
  return ACTION_TYPE_LABELS[actionType] || fallback;
}

export function getResearchFacets(entry: ApiKnowledgeEntry): {
  region: string;
  industry: string;
  actionType: string;
} {
  const text = collectResearchText(entry);
  const isAction = entry.source_domain === "research.action_card";
  return {
...
export function getResearchPerspectiveScore(entry: ApiKnowledgeEntry, perspective: ResearchPerspective): number {
  if (perspective === "all") {
    return 1;
  }
  ...
  const actionType = ((entry.metadata_payload || {}) as { card?: { action_type?: string } }).card?.action_type || "";
  ...
  if (perspective === "regional") {
    ...
    if (entry.source_domain === "research.report") score += 2;
  ...
  if (perspective === "client_followup") {
    ...
    if ((report?.client_peer_moves || []).length) score += 2;
  ...
  if (perspective === "bidding") {
    ...
    if ((report?.competition_analysis || []).length) score += 1;
    return score;
  }
  ...
  if (perspective === "ecosystem") {
    ...
    if ((report?.winner_peer_moves || []).length) score += 2;
...
export function buildFacetOptions<T extends string>(values: T[], allLabel: string): string[] {
